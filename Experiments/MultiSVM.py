...
def MultiSVM():
    ...
    params = [
        {'C': [0.0001, 0.001, 0.01, 0.1, 10, 100, 1000, 10000], 'kernel': ['rbf'], 'class_weight': ['balanced'],
         'decision_function_shape': ['ovo', 'ovr']},
        {'C': [0.0001, 0.001, 0.01, 0.1, 10, 100, 1000, 10000], 'kernel': ['linear'], 'class_weight': ['balanced'],
         'decision_function_shape': ['ovo', 'ovr']},
        {'C': [0.0001, 0.001, 0.01, 0.1, 10, 100, 1000, 10000], 'kernel': ['poly'],
         'class_weight': ['balanced'], 'decision_function_shape': ['ovo', 'ovr']},
        {'C': [0.0001, 0.001, 0.01, 0.1, 10, 100, 1000, 10000], 'kernel': ['sigmoid'], 'class_weight': ['balanced'],
         'decision_function_shape': ['ovo', 'ovr']},
    ]

    grid = GridSearchCV(estimator=svm.SVC(), param_grid=params, cv=cv_folds, n_jobs=4, scoring='f1_micro')
    grid.fit(X_train_validate, y_train)
    best_model = svm.SVC(C=grid.best_params_['C'],
                            kernel=grid.best_params_['kernel'],
                            decision_function_shape=grid.best_params_['decision_function_shape'],
    ...
    for train_ind, test_ind in cv_folds:
        ...
        y_multi_test = label_set.vec2mat(y_test, clf_args.target_set)
        y_multi_predict = label_set.vec2mat(y_predict, clf_args.target_set)
